...
- **알림 피로 규칙은 `policy.py` 한 곳** — 강도(`push`/`silent`/`feed`)·하루 push 상한·무음 시간(23:00~08:00 KST)·클러스터당 하루 1건. 어댑터나 잡에 이 판단을 복제하지 않는다.
- **발송 기록** — 성공·실패 모두 `notifications` 에 남긴다 (`channel`, `level`, `message_id`, `error`). 실패한 항목은 `FAILED` 로 두고 재시도 여부는 잡이 정한다.
