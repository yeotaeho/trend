...
import 에 `from app.pipeline.embedding import EmbeddingDimError, embed_pending` 를 추가한다. `EmbeddingDimError` 는 설정 오류라 로그에 더해 `send_ops_alert` 로도 보낸다. 아래 블록이 위 `try` 의 `except` 절을 대체한다.
...
async def main() -> None:
    ...
    try:
        ...
        print("모델 불일치 행이 남아 있는 동안 파이프라인 잡은 스스로 멈춘다 (pipeline.paused_for_reembedding).")
...
**모델 교체 중 파이프라인 정지.** 백필이 도는 동안 파이프라인이 돌면 옛 벡터와 새 벡터가 섞여 비교된다. 운영 절차에 의존하지 않고 코드로 막는다. `app/jobs/pipeline.py` 에 아래 가드를 두고 `run_pipeline` 시작에서 부른다 (Task 1.6 에서 넣고 2.6 의 전면 교체본에도 들어 있다).

```python
async def _reembedding_in_progress(session: AsyncSession) -> bool:
    """embedding_model 이 현재 설정과 다른 행이 하나라도 있으면 백필 중이다. 섞어 비교하지 않는다."""
    model = get_settings().embedding_model
    stmt = select(Item.id).where(Item.embedding_model.is_not(None), Item.embedding_model != model).limit(1)
    return (await session.execute(stmt)).first() is not None
```

가드가 성립하는 이유는 Task 1.3 의 `embed_pending` 이 불일치 행을 NULL 로 되돌리지 않고 `embedding IS NULL OR embedding_model != :model` 을 그대로 대상으로 잡기 때문이다. 백필이 끝날 때까지 불일치 행이 남아 가드가 유지되고, 마지막 행이 재계산되는 순간 가드가 풀린다. 새 항목은 수집 때 현재 모델로 계산되므로 불일치를 만들지 않는다.

- [ ] **Step 3: 백필 실행**

Run: `PYTHONIOENCODING=utf-8 uv run python scripts/backfill_embeddings.py`
Expected: 마지막 줄 `완료: 4xxx건 계산, 남은 대상 0건`.
...
항목의 `embedding` 이 NULL 이면 `q` 가 NULL 이라 `<=>` 결과도 NULL 이고 두 쿼리 모두 빈 결과다. 호출자는 그 경우를 `dedupe_skipped` 로 기록한다.

**벡터 쓰기 경로는 하나다.** 벡터를 DB 에 넣는 코드는 `update(Item).values(embedding=...)` (Task 1.3 의 `embed_pending`) 뿐이다. raw SQL 로 벡터를 삽입하지 않는다. 테스트도 같은 경로를 쓴다.
...
- [ ] **Step 6: 통합 테스트 — 쿼리 A 가 탈락 후보 너머의 생존 항목을 찾는가**

```python
# tests/integration/test_dedupe_db.py
...
async def test_query_a_prefers_survivor_over_more_similar_dropped():
    ...
    async with SessionLocal() as s, s.begin():
        ...
        def item(key: str, status: str) -> Item:
            return Item(
                source_id=src.id, external_id=key, url=f"https://t/{key}",
                url_normalized=f"https://t/{key}", url_hash=f"dd-{key}", title=key,
                published_at=now, status=status,
            )

        target, dropped, survivor = item("target", "NEW"), item("dropped", "DROPPED"), item("survivor", "SENT")
        s.add_all([target, dropped, survivor])
        await s.flush()
        for it, v in ((target, vec(1.0, 0.0)), (dropped, vec(1.0, 0.01)), (survivor, vec(1.0, 0.2))):
            await s.execute(update(Item).where(Item.id == it.id).values(embedding=v, embedding_model="t"))
...
Run: `TEST_DATABASE_URL=<dev> uv run pytest tests/integration/test_dedupe_db.py -v`
Expected: 1 passed. 통합 테스트 골격(`tests/integration/conftest.py`)은 Task 2.4 에 있다. 여기서 먼저 만든다.

- [ ] **Step 6: 커밋**

```bash
git add app/pipeline/dedupe.py app/config.py config/rules.yaml tests/test_dedupe.py tests/integration
...
`_reembedding_in_progress` 는 Task 1.4 에 정의돼 있다. import 에 `from app.pipeline.embedding import EmbeddingDimError, embed_pending`, `from app.notify.discord import send_ops_alert`, `from app.config import get_settings` 를 추가한다.
...
`uuid` import 는 쓰지 않으므로 뺀다. import 에 `from app.config import Rules, get_rules, get_settings`, `from app.pipeline.embedding import EmbeddingDimError, embed_pending` 을 맞추고, `_reembedding_in_progress` (Task 1.4) 를 이 파일에 둔다. `TriageEntry.idx` 에 `item.id` 를 그대로 쓰므로 프롬프트의 `[idx]` 가 항목 id 다. 응답에 낯선 숫자가 오면 배치 실패로 잡힌다.

`llm.body_for_judge` 는 이 태스크에서 `app/pipeline/llm.py` 에 추가한다. 탐색 슬롯(Task 3.3)도 같은 함수를 써서 일반 판정과 같은 재료로 판정한다.

```python
async def body_for_judge(item: Item) -> tuple[str | None, bool]:
    """본문이 짧으면 원문을 받아 보강한다. (본문, 보강 실패 여부). 보강 결과는 summary_raw 에 남긴다."""
...
`from app.db.models import Item` 을 llm.py 에 추가한다.
...
async def test_nearest_feedback_finds_identical_vector():
    async with SessionLocal() as s, s.begin():
        ...
        a = Item(source_id=src.id, external_id="a", url="https://t/a", url_normalized="https://t/a",
                 url_hash="fb-a", title="A", published_at=now)
        b = Item(source_id=src.id, external_id="b", url="https://t/b", url_normalized="https://t/b",
                 url_hash="fb-b", title="B", published_at=now)
        s.add_all([a, b])
        await s.flush()
        await s.execute(
            update(Item).where(Item.id.in_([a.id, b.id])).values(embedding=VEC_LIST, embedding_model="t")
        )
...
위 테스트의 벡터 삽입은 raw SQL 이 아니라 Task 1.5 통합 테스트와 같은 `update(Item).values(embedding=VEC_LIST, embedding_model="t")` 경로로 쓴다. `VEC` 문자열 대신 `VEC_LIST = [0.03] * 1024` 를 두고, `published_at` 은 `datetime.now(UTC)` 로 넣는다. raw SQL 로 `CAST(:v AS vector)` 를 쓰면 asyncpg 가 파라미터를 vector 로 추론해 코덱이 없어 실패한다.
...
**Interfaces:**
- Produces: `Level.EXPLORE = "explore"`, `EXPLORE_PREFIX = "🧪 실험 · 경계 항목"`, `async _explore(session, rules, notifier, *, now: datetime) -> bool`
- Consumes: `llm.body_for_judge(item)` (Task 2.6), `reserve_call("explore")`, `nearest_feedback`
...
async def _explore(
    ...
    now 는 호출 직전에 읽은 시각이다. 인자로 받아야 자정 경계 테스트가 된다.
    """
    cfg = rules.notify
    ...
    if await reserve_call("explore") is None:
        return False
    examples = format_examples(await nearest_feedback(session, item.id, k=3))
    result = await llm.judge(rules, source=source.name, title=item.title, body=body, examples=examples)
    session.add(
        Decision(
            item_id=item.id, stage=Stage.LLM.value, passed=result.verdict.worth_notifying,
            score=item.score,
            details={
                "explore": True, "enrich_failed": enrich_failed,
                "importance": result.verdict.importance, "tags": result.verdict.tags,
            },
        )
    )
    summary = Summary(
...
```python
        try:
            await _explore(session, get_rules(), notifier, now=datetime.now(UTC))
        except Exception as exc:
            await session.rollback()
            log.warning("notify.explore_failed", error=str(exc))
```

같은 태스크에서 `run_notify` 의 `now` 재사용도 고친다. 잡 시작에 한 번 만든 `now = datetime.now(UTC)` 를 지우고, 루프 안의 `decide(...)` 호출 직전에 `now = datetime.now(UTC)` 를 읽는다. 23:59 에 시작해 자정을 넘기면 push 상한과 무음 시간이 발송 시점의 날짜로 판정돼야 한다.
...
```python
        for _ in range(BATCH_SIZE):
            ...
            visited.add(item.id)
            # 매 항목마다 새로 읽는다. 잡 시작 시각을 재사용하면 자정을 넘길 때 상한 검사 날짜가 어긋난다.
            now = datetime.now(UTC)
            verdict = await decide(session, cfg, importance=summary.importance, now=now)
```

- [ ] **Step 3b: 탐색 후보 통합 테스트**

```python
# tests/integration/test_explore_db.py
# 탐색 후보 선택 통합 테스트 — 임계값 바로 아래·24시간 이내·Summary 없음·마지막 score 탈락인 최고점 항목
...
async def test_explore_candidate_picks_highest_in_band_without_summary():
    ...
    async with SessionLocal() as s, s.begin():
        ...
        def item(key: str, score: float, age_h: int = 1) -> Item:
            return Item(
                source_id=src.id, external_id=key, url=f"https://t/{key}",
                url_normalized=f"https://t/{key}", url_hash=f"ex-{key}", title=key,
                published_at=now - timedelta(hours=age_h), status="DROPPED", score=score,
            )

        best, low, old, summarized, passed_later = (
            item("best", 0.44), item("low", 0.30), item("old", 0.44, age_h=30),
            item("summarized", 0.43), item("passed_later", 0.44),
        )
        ...
        s.add(Summary(item_id=summarized.id, title_ko="x", summary_ko="", importance=1,
                      worth_notifying=False, model="m"))
...
Run: `TEST_DATABASE_URL=<dev> uv run pytest tests/integration/test_explore_db.py -v`
Expected: 1 passed
...
```bash
git add app/schemas.py app/notify/discord.py app/jobs/notify.py tests/test_discord.py tests/integration/test_explore_db.py
git commit -m "feat: 탐색 슬롯 — 하루 1건 경계 항목을 판정해 🧪 카드로 발송, 발송 잡의 시각을 항목마다 읽음"
```
...
**Interfaces:**
- Produces: `adjust_trust(base: float, useful: int, useless: int, *, prior_weight: int = 10) -> float`, `precision(useful: int, useless: int) -> float | None`, `score_band(score: float, width: float = 0.05) -> float`
...
# 소스 신뢰도 보정과 리포트 집계 — 베이즈 평활(사전 가중치 10 = 라벨 10건이면 반반), 정밀도, 점수 구간
...
Run: `uv run pytest tests/test_trust.py -v` → 5 passed

```bash
git add app/pipeline/trust.py tests/test_trust.py
git commit -m "feat: 소스 신뢰도 베이즈 보정, 리포트 집계 순수 함수"
```
...
# 탈락 "건수" 는 FUNNEL 이 stage·reason 별로 이미 센다. 여기는 정책 문장을 고칠 근거인 선별 이유 "문장" 표본이다.
...
def band_table(rows: Sequence[Any]) -> list[tuple[float, int, int, int, float | None]]:
    """(score, verdict) 행 → 구간별 (band, sent, useful, useless, precision). 구간 나누기는 순수 함수."""
...
async def main(apply: bool) -> None:
    try:
        async with session_scope() as session:
            ...
            table(
                "소스별 발송·👍·👎·정밀도",
                [(r.name, r.trust_score, r.trust_adjusted, r.sent, r.useful, r.useless,
                  precision(r.useful, r.useless)) for r in sources],
            ...
            table("저관련도 선별 이유 표본 10", (await session.execute(LOW_RELEVANCE_SAMPLES, p)).all())
...
- **문서 대조가 필요한 외부 형식** — Voyage/OpenAI 요청 본문(1.3), Anthropic `system` 블록 배열과 `cache_control`(2.5), pgvector 의 `CAST(text AS vector)` 동작(1.1). 구현 직전에 공식 문서로 확인하고, 다르면 코드를 문서에 맞춘다.
- **Codex 1차 리뷰 반영 (2026-09-06)** — 선별 재시도도 예약(2.6 `_triage_once`), 통합 테스트의 벡터 삽입은 ORM 경로만(1.5·3.1), 항목 실패 카운트는 add 전에 셈(2.6), 모델 교체 중 파이프라인 가드(1.4·1.6·2.6), 파이프라인 경로의 차원 오류 알림(1.6·2.6), `Retry-After`(1.3), 쿼리 A·탐색 후보 통합 테스트(1.5·3.3), 탐색도 보강 공유(`body_for_judge`, 2.6·3.3), `_explore(now=)` 시그니처와 발송 잡의 항목별 시각(3.3), 리포트의 정밀도·구간·표본(4.1·4.2). 설계서 §11 의 `reserve_call -> bool` 은 `str | None`(batch_id) 로 정정.
