...
- 모르는 키(`updated_at` 포함)를 보내면 422. 저장된 덮어쓰기 중 검증에 실패하는 옛 섹션(YAML 키가 바뀐 경우)은 저장 때 지워진다. 그래도 합친 결과가 검증에 실패하면 저장하지 않고 422 이며 `details.reason` 에 이유가 있다 (05·06 저장도 같다).
